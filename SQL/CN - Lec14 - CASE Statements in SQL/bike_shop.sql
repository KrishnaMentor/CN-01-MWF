...
-- CASE With Aggregation Functions
SELECT 
	territoryKey,
    SUM(
		CASE 
...
-- CASE Challenge - JOIN Concepts
-- Customers return analysis

SELECT 
	Customers.CustomerKey,
    Concat(FirstName , ' ', LastName) AS CustomerName,
    SUM(COALESCE(ReturnQuantity,0)) AS TotalReturns,
    CASE
		WHEN SUM(COALESCE(ReturnQuantity,0)) > 10 THEN 'Frequent Returner'
        WHEN SUM(COALESCE(ReturnQuantity,0)) BETWEEN 1 AND 10 THEN 'Occasional Returner'
        ELSE 'Non Returner'
    END As ReturnBehaviour
FROM Customers
LEFT JOIN `sales-2015`
ON Customers.CustomerKey = `sales-2015`.CustomerKey
LEFT JOIN Returns
ON Returns.ProductKey = `sales-2015`.ProductKey
GROUP BY 1,2
ORDER BY TotalReturns DESC
LIMIT 50;


-- AGGREGATION WITH CASE[CategoryName] -- Region
-- In which what category are having highest Returns.
